...
module('Acceptance | songs', function (hooks) {
  ...
  test('Sort songs in various ways', async function (assert) {
    ...
    assert
      ...
      .hasText(
        'Elephants',
        'The first song is the on that comes first in the alphabet'
      );
    assert
    ...
    assert
      ...
      .hasText(
        'Elephants',
        'The last song is the one that come first in the alphabet'
      );
    assert.ok(
    ...
    assert
      .dom('[data-test-rr=song-list-item]:first-child')
      .hasText('Mind Eraser, No Chaser', 'The song is the lowest rated');
  ...
  test('Search Songs', async function (assert) {
    ...
    assert
      ...
      .exists({ count: 2 }, 'The song matching the search term are displayed');
